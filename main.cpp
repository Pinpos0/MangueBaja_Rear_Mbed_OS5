...
//              Thread::wait(1);
            ...
            case FUEL_ST:
                ...
                data.flags |= (fuel_counter > NORMAL_THRESHOLD) ? (0x01 << 3) : (0x01 << 3);
...
#endif
#ifdef MB2
                radio.send((uint8_t)BOXRADIO_ID2, &data, sizeof(packet_t), true);     // request ACK with 1 retry (waitTime = 40ms)
#endif
                if (t.read_ms() - imu_last_acq > 500) {
...
void filterMessage(CANMsg msg)
{
    ...
    if (msg.id == THROTTLE_ID) {
        ...
        msg >> data.imu.dps_x >> data.imu.dps_y
            >> data.imu.dps_z;
    } else if (msg.id == SPEED_ID) {
        msg >> data.speed;
    } else if (msg.id == VOLT_ID) {
        msg >> data.volt;
    } else if (msg.id == TEMP_CVT) {
        msg >> data.cvt;
    } else if (msg.id == SOC_ID) {
        msg >> data.soc;
    } else if (msg.id == LAT_ID) {
        msg >> data.lat;
    } else if (msg.id == LNG_ID) {
        msg >> data.lng;
    }
}
